...
- **Production build run for the first time: passed.** 448 outputs, which counts generated metadata and OG images rather than 448 distinct content pages. Every public content route is static or SSG — the pre-render guarantee in CLAUDE.md holds. The dynamic routes are all admin, API or signed-in ones (`/manage`, `/account`, `/resources`, `/preview`, `/api/*`, the calendar feeds).
